...
class _HomePageState extends State<HomePage> {
  ...
  String _getStatusString(Job job){
    return job.finalized && !job.staffConfirmed.contains(userID) ? "DENIED" :
    (job.staffConfirmed.contains(userID) ? "CONFIRMED" : "AWAITING");
  }

  Color _getStatusColor(Job job){
    return job.finalized && !job.staffConfirmed.contains(userID) ? colorDenied:
    (job.staffConfirmed.contains(userID) ? colorConfirm : colorAwait);
  }
                ...
                content: SizedBox(
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                          ...
                          children: [
                            _tabIndex == applied ? Text("Status: ${_getStatusString(job)}") : Container(),
                            ...
                            ElevatedButton(
                              ...
                              child: Text(_tabIndex == applied ? "Cancel" : "Apply", style: whiteText),
                              onPressed: () async{
        ...
        child: Center(
          child: Column(
              children: <Widget>[
                ...
                Text("Date of Birth: ${currentUser.getDob()}"),
                TextButton(
                    onPressed: () async{
                      await textEditDialog(context, "Change Postcode", currentUser.email).then((value){
                        setState((){
                          currentUser.postcode = value;
                        });
                      });
                    },
                    child: Text("Postcode: ${currentUser.postcode}")
                ),
                TextButton(
                    onPressed: () async{
                ...
                Row(
                  children: [
                    Text("I Own Transport: ${currentUser.ownsTransport}"),
                    Checkbox(
